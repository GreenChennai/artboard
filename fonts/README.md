# artboard 字体库总目录

> 两级筛查法:先按「分类」选定大类 → 点进对应 `<目录>/INTRO.md` 细读气质与适用场景 → 定稿 ≤3 款(标题/正文/数字各司其职)。
> 新增字体:建目录放字体文件 + INTRO.md,然后跑 `scripts/add_font.py <目录名> --name 显示名 --category 分类 --tags 关键词`。

| 目录 | 字体 | 分类 | 气质关键词 | 字重/变体 |
|---|---|---|---|---|
| [source-han-sans]source-han-sans/INTRO.md | 思源黑体 | 正文可读·大标题 | 现代,中性,全能,数据感,可变字重 | 可变 100-900 |
| [source-han-serif]source-han-serif/INTRO.md | 思源宋体 | 文艺人文·杂志标题 | 人文,书卷,高级,编辑部 | 可变 200-900 |
| [smiley-sans]smiley-sans/INTRO.md | 得意黑 | 标题冲击 | 冲刺,动势,促销,竞技 | Oblique 单字重 |
| [lxgw-wenkai]lxgw-wenkai/INTRO.md | 霞鹜文楷 | 文艺人文·手写批注 | 温润,手写,信笺,小清新 | Regular 单字重 |
| [zcool-kuaile]zcool-kuaile/INTRO.md | 站酷快乐体 | 标题冲击·可爱手账 | 俏皮,圆润,零食感,童趣 | Regular 单字重 |
