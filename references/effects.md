# artboard 视觉特效分册(43 式:fx- 34 + tx- 9,纯 CSS/SVG,静态渲染安全)

> **使用纪律:一图 ≤3 种特效;特效服务层级,不堆砌。**
> 纵深/厚度/伪3D(挤出/透视/遮挡造空间)→ `depth-3d.md`,本册不收纵深类配方。
...
> 本节配方是「怎么写」;这些几何背后的**构成语言**(等分/栅格/布尔/负形怎么想)见
> `vector-drawing.md §4.2`;要矢量交付时 conic 类配方(如 fx-starburst)的替代画法
> (点环/方块放射,内联 SVG 会栅格化)见 `vector-drawing.md §4.5`。
