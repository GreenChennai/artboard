...
| 层 | 来源 | 内容 |
|---|---|---|
| 背景 | 未打标元素(html/body 画布底色) | 每层都带,合成时在最底 |
| 图形 | 有底色/边框/阴影的装饰元素、**面板底色克隆** | 卡片壳、徽章、放射纹 |
| 图片 | img/svg/canvas/picture/video | 照片与插画 |
| 蒙层 | 图片**之后**的同级渐变遮罩(veil) | 必须盖在照片上,单独成层保住叠序 |
| 文字 | 有直接文字的元素(底色随行) | 全部文案 |

- **ai.pdf 的图层 = PDF OCG(可选内容组)**:Acrobat/浏览器/Foxit 里可见并可开关;
  字段照抄 Illustrator 原生写法(/Intent+/Usage),AI 兼容性最优。
- **面板底色克隆**:有底色又含内容的元素(卡片),底色被剥离到克隆元素归入图形层,
  本体转纯结构——文字层与图形层才能干净分离(克隆用 div,防被海报的 i/b 选择器误中)。
- 手动指定:元素加 `data-ai-layer="背景|图形|图片|文字"` 强制归层。
...
| 输入 | 处理 |
|---|---|
| PDF / .ai | pdftohtml -xml(文字坐标/字号/颜色 + 图片位)+ pdftocairo -svg(矢量底景) |
